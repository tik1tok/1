...
function run($ig) {
    try {
        ...
        output('Type integer value without spaces from 1 to 800 000 stories/day or 0 for maximum possible speed.');
        output('We recommend you set 400 000 stories/day. This speed works well for a long time without exceeding the limits.');
        ...
        output('If you are using another type of automation, we recommend to you reducing masslooking speed and find your own golden ratio.');
        $speed = (int)getVarFromUser("Speed");

        if ($speed > 800000) {
            do { 
                output("Speed value is incorrect. Type integer value from 1 to 800 000 stories/day.");
        ...
        output('Mark as seen only fresh stories, which posted no more than X hours ago.');
        output('X - is integer value from 1 to 23.');
        output('Type 0 to skip this option.');
        output('This option will reduce speed, but can increase results of masslooking.');
        $fresh_stories_range = (int)getVarFromUser("X");

        if ($fresh_stories_range > 23) {
            do { 
                output("Fresh stories range value is incorrect. Type integer value from 1 to 23.");
                output('Type 0 for skip this option.');
                $fresh_stories_range = (int)getVarFromUser("X");
        ...
        masslooking_v2($data, $ig, $delay, $fresh_stories_range);
 ...
 */
function masslooking_v2($data, $ig, $delay, $fresh_stories_range) {
            ...
            try {
                ...
                if (!empty($d['begin_gf'])) {
                    ...
                    if (($current_time - $d['begin_gf']) < 7) {
                        $sleep_time = 7 - intval($current_time - $d['begin_gf']);
                        sleep($sleep_time);
                    }
                }
                ...
                try {
                    ...
                    output("@" . $d['username'] . " not found or maybe user blocked you (login to Instagram website or mobile app and check that).");
                    unset($data[$key]);
                    continue;
                } catch (Exception $e){
                    // output("Exception after getFollowers() / Debug: getReelsMediaFeed (" . $counter1 . "), markMediaSeen (" . $counter2 . ")");
                    throw $e;
                }

                // DEBUG - BEGIN
                // $followers_json = json_decode($followers, true);
                ...
                foreach ($followers->getUsers() as $follower) {
                    // Check is account private, with anonymous profile picture or verified
                ...
                // Calculate number of followers which match criteria
                $number = null;
                ...
                if (!empty($number)) {
                    ...
                    do { 
                        ...
                        // Double check the user ID's list
                        if (empty($ids)) {
                            break;
                        }
                        ...
                        try {
                            try {
                                ...
                                // Debug
                                // output("Exception after getReelsMediaFeed() / Debug: getReelsMediaFeed (" . $counter1 . "), markMediaSeen (" . $counter2 . ")");
                                throw $e;
                            }
                                ...
                                foreach ($reels as $r) {
                                    ...
                                    if ($st_count >= 200) {
                                        ...
                                        try {
                                            ...
                                            // Debug
                                            // $mark_seen_resp_json = json_decode($mark_seen_resp, true);
                                            // output(var_dump($mark_seen_resp_json));
                                        } catch (Exception $e){ 
                                            // Debug
                                            // $mark_seen_resp_json = json_decode($mark_seen_resp, true);
                                            // output(var_dump($mark_seen_resp_json));
                                            // output("Exception after markMediaSeen() at 'if (st_count >= 200)' / Debug: getReelsMediaFeed (" . $counter1 . "), markMediaSeen (" . $counter2 . ")");
                                            throw $e;
                                        } 
                                        $begin_ms = time();
                            ...
                            if (($st_count > 0) && $last && $data[$key]['max_id'] == null) {
                                try {
                                    ...
                                    // $mark_seen_resp_json = json_decode($mark_seen_resp, true);
                                    // output(var_dump($mark_seen_resp_json));
                                    // output("Exception after markMediaSeen() at 'last && data[key]['max_id'] == null' / Debug: getReelsMediaFeed (" . $counter1 . "), markMediaSeen (" . $counter2 . ")");
                                    throw $e;
                                } 
                            ...
                            output("Throttled by Instagram because of too many API requests.");
                            output("12 hours rest for account started because you reached Instagram daily limit for masslooking.");
                            sleep(43200);
                        } catch (Exception $e){
                            output($e->getMessage());
                            sleep(7);
    ...
    masslooking_v2($targets, $ig, $delay, $fresh_stories_range);
}
